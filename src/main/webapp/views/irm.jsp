    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header border-bottom badge bg-primary pt-1 pb-1">
          <h4 class="card-title text-white"><span id="incidentValue">IRM</span> FORM</h4><p id="demo"></p>
        
        </div>
      <!--   <div class="card-header">
        </div> -->
        <div class="card-body mt-1">
          <form class="form " action="<%=request.getContextPath() %>/irm-submit" id="irmForm" name="irmForm" method="post" class="form-horizontal" role="form" enctype="multipart/form-data">
   ...
   <!--            <div class="col-md-6 col-6"> 
              ...
              <select class="select2 form-select" id="select2-incident_category-multiple" name="incident_category"  data-placeholder="Select Incident Category" multiple >
	              ...
	              <option value="Chemical Spill or Overflow">Chemical Spill or Overflow</option>
					...
					<option value="Life Saving Rules">Life Saving Rules</option>
...
<!-- 			 
            ...
            <div class="col-12 " style="text-align: center;">
               <div>
                <button type="button" class="btn btn-primary me-1 btn-outline-primary btn-page-block-custom waves-effect " id="sBtn" onclick="submitIRM();">Submit</button>
               	<a href="<%=request.getContextPath()%>/irm" class="btn btn-outline-secondary" >Cancel</a>
                </div>
              </div>
    ...
    <script>
      ...
      function getIRMFilterList() {
	        ...
	        if ($.trim(company_code) == "") {
	            ...
	            $.ajax({
	                url: "<%=request.getContextPath()%>/ajax/getIRMFilterList",
      ...
      function getStatusFilterList() {
	    ...
	    function submitIRM(){
	    	if(validator.form()){ // validation perform
	    		var typeOfIncident = $('#incidentValue').text();
	     	    $.blockUI({
	   		        message:
	   		          '<div class="d-flex justify-content-center align-items-center"><p class="me-50 mb-0">Adding '+typeOfIncident+', Please wait...</p> <div class="spinner-grow spinner-grow-sm text-white" role="status"></div> </div>',
	   		        css: {
	    ...
	    function roleMapping(){
	        ...
	        if ($.trim(project_code) != "" && $.trim(department_code) != "" && $.trim(incident_type) != "" ) {
	            ...
	            $.ajax({
	                url: "<%=request.getContextPath()%>/ajax/getRoleMappingforIRMForm",
	                data: myParams, cache: false,async: false,
	                success: function (data) {
	                    if (data.length > 0) {
	                        $.each(data, function (i, val) {
	    ...
	    function showFrom(incident){
	    	if(incident != ''){
	    		...
	    		// $("#approverForIncident").html('No Reviewer Assigned').css("color", "orange");
		    	 ...
		    	 $('#incidentValue').html(incident_name).css("text-transform", "uppercase");
	    	}else{
